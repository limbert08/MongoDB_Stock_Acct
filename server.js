...
var MONGODB_URI = process.env.MONGODB_URI || "mongodb://localhost/acctsdb";
...
app.get("/all", function(req, res) {
  ...
  db.transactions.find({}, function(error, found) {
...
app.get("/name", function(req, res) {
  ...
  db.transactions.find().sort({ tran_symbol: 1 }, function(error, found) {
    // Log any errors if the server encounters one
    if (error) {
      console.log(error);
    }
...
app.get("/weight", function(req, res) {
  ...
  db.transactions.find().sort({ tran_id: -1 }, function(error, found) {
    // Log any errors if the server encounters one
    if (error) {
      console.log(error);
    }
